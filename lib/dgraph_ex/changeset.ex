defmodule DgraphEx.Changeset do
  ...
  """
  def uncast(%Cs{module: module, model: model, changes: changes} = cs) do
    cond do
      !is_list(cs.errors) ->
        raise %ArgumentError{
      ...
      is_valid?(cs) -> 
        {:ok, struct!(module, do_apply_changes(model, changes))}
      true ->
        {:error, cs}
    end
  end

  defp do_apply_changes(model, changes) do
    model
    |> Enum.reduce(model, fn
      ({key, _}, model_acc) ->
        if Map.has_key?(changes, key) do
          changed_value = Map.get(changes, key)
          Map.put(model_acc, key, changed_value)
        else
          model_acc
        end
    end)
  ...
  def validate_required(%Cs{changes: changes} = cs, required_fields) do
    validate_required_errors(changes, required_fields)
    |> Enum.reduce(cs, fn
      (err, acc_cs) -> put_error(acc_cs, err)
    end)
  end

  defp validate_required_errors(changes, required_fields) do
    required_fields
    |> Enum.map(fn key -> {key, Map.get(changes, key)} end)
    |> Enum.reduce([], fn
  ...
  def validate_type(%Cs{changes: changes} = cs, field_name, type) when is_atom(type) do
    value = Map.get(changes, field_name)
  ...
  def validate_type(%Cs{changes: changes} = cs, field_name, types) when is_atom(field_name) and is_list(types) do
    value = Map.get(changes, field_name)
