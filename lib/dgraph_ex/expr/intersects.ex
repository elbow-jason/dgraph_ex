defmodule DgraphEx.Expr.Intersects do
  ...
  defmacro __using__(_) do
    quote do
      ...
      def intersects(label, [[ [x, y] | _] |_] = geo_json) when is_atom(label) and is_float(x) and is_float(y) do
  ...
  def render(%Intersects{label: label, geo_json: [[ [x, y] | _] |_] = geo_json }) when is_atom(label) and is_float(x) and is_float(y) do
